...
export const computeButtonStyles = (
 ...
 `;
  ...
  const blockButtonClass: SerializedStyles | null =
    block && radius !== "circle"
      ? css({ display: "block", width: "100%" })
      : null;
  const buttonVariantClass: SerializedStyles = buttonVariantMappings[variant];
  const buttonSizeClass: SerializedStyles =
    radius === "circle"
      ? circleButtonSizeMappings[size]
      : buttonSizeMappings[size];
  const buttonRadiusClass: SerializedStyles = buttonRadiusMappings[radius];

  const buttonClasses: SerializedStyles[] = [
    buttonBaseClass,
    buttonVariantClass,
    buttonSizeClass,
    buttonRadiusClass,
